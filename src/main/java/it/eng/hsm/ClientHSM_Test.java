...
class ClientHSM_Test implements ClientHSM {
    ...
    // private byte[] signsPAdES(RemoteSignatureCredentials credentials, byte[] fileToSign) throws
    // HSMException {
    // byte[] signedBytes = null;
    //
    // SignatureFlags flags = new SignatureFlags();
    ...
    //
    // try {
    // signedBytes = client.signP7M(credentials, fileToSign, false, false, Constants.DIGEST_TYPE,
    // null, flags);
    // } catch (RemoteSignatureExceptionException er) {
    // String errorCod = er.getMessage();
    //
