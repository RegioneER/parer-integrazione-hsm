...
import it.eng.hsm.prod.itagile.firmaremota.ws.RemoteSignatureCredentials;
import it.eng.hsm.prod.itagile.firmaremota.ws.RemoteSignatureXadesParams;
import it.eng.hsm.prod.itagile.firmaremota.ws.SignatureFlags;
import it.eng.hsm.prod.itagile.firmaremota.ws.RemoteSignature;
import it.eng.hsm.prod.itagile.firmaremota.ws.RemoteSignatureExceptionException;
import it.eng.hsm.prod.itagile.firmaremota.ws.RemoteSignature_Service;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * The implementation of {@link ClientHSM} for the endpoint of Prod RER.
 *
 * @author Moretti_Lu
 *
 */
class ClientHSM_Prod implements ClientHSM {

    private final static Logger logger = LoggerFactory.getLogger(ClientHSM_Prod.class);

    private RemoteSignature_Service service;
    private RemoteSignature client;

    public ClientHSM_Prod() throws HSMException {
	try {
	...
	logger.info("Client HSM - endpoint PRODUZIONE");
    }

    /**
     * Returns the respective {@link RemoteSignatureCredentials} of the <code>user</code> (an
     * instance of {@link HSMUser})
     *
     * @param user
     *
     * @return
	...
	try {
	    ...
	    logger.info("Test HSM - " + session.getUser().getUsername()
		    + "'s session signature closed");
	} catch (Exception e) {
	    throw new HSMException(e);
	}
    }
    ...
    private byte[] signsP7M(RemoteSignatureCredentials credentials, byte[] fileToSign,
	    boolean marcaTemporale) throws HSMException {
	byte[] signedBytes = null;

	SignatureFlags flags = new SignatureFlags();
	// No marca temporale
	flags.setTimestamp(marcaTemporale);
	flags.setCadesDetached(false); // La firma d'ora in poi sarà gestita sempre in modalità
				       // embedded

	try {
	    signedBytes = client.signP7M(credentials, fileToSign, false, false,
		    Constants.DIGEST_TYPE, null, flags);
	...
	// return new ConfigurationXAdES(XAdES.PROFILES.BES, XAdES.TYPE.DETACHED,
	// XAdES.VALIDATION.T);
	...
	result = signsXAdES(getRemoteSignatureCredentials(user), fileToSign,
		marcaTemporale ? getConfigurationXAdESWithTimestamp()
    ...
    private byte[] signsXAdES(RemoteSignatureCredentials credentials, byte[] fileToSign,
	    ConfigurationXAdES conf) throws HSMException {
	...
	if (conf != null) {
	    ...
	    params.setValidationData(
		    conf.getValidation() != null ? conf.getValidation().name() : null);
	...
	try {
	    ...
	    signedBytes = client.signXAdES(credentials, fileToSign, Constants.DIGEST_TYPE, null,
		    params);
