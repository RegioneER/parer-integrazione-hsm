...
/**
 * This class contains a main method to verify what the HSM does when the OTP period expires. The
 * method signs the same file an infinite time increasing the waiting interval between one signing
 * and the next one.
 *
 * @author Moretti_Lu
 */
public class LoadTest {
	...
	try {
	    fileToSign = IOUtils
		    .toByteArray(Utils.getFileFromResourcesAsStream(TestUtils.TEST_FILE));
	} catch (Exception e) {
	...
	while (true) {
		...
		logger.info("Firmati " + counter + " file in " + durationMin + " minuti ["
			+ durationSec + " secondi o " + durationMs
			+ " millisecondi] - dopo la pausa di " + counter + " secondi");
